...
class RemoteAccessibilityService : AccessibilityService() {
    ...
    private fun dispatchDragStrokeLocked(
        ...
        val accepted = dispatchGesture(
            ...
            object : GestureResultCallback() {
                override fun onCompleted(gestureDescription: GestureDescription?) {
                    gestureHandler.post {
                        synchronized(dragLock) {
                            ...
                            if (active == null || active.generation != state.generation) {
                                return@synchronized
                            }
                ...
                override fun onCancelled(gestureDescription: GestureDescription?) {
                    gestureHandler.post {
                        synchronized(dragLock) {
                            if (dragState?.generation == state.generation) {
                                dragState = null
                            }
                        }
                    }
        ...
        if (!accepted && dragState?.generation == state.generation) {
            dragState = null
        }
    }
    ...
    fun moveFocusedCursor(action: String): Boolean {
        ...
        val cursor = when (action) {
            "left" -> if (start != end) {
                ...
                current.offsetByCodePoints(start, -1)
            } else {
                0
            }
            "right" -> if (start != end) {
                ...
                current.offsetByCodePoints(end, 1)
            } else {
                current.length
            }
            "home" -> 0
    ...
    fun pressFocusedEnter(): Boolean {
        ...
        if (Build.VERSION.SDK_INT >= 30) {
            ...
            if (
                node.actionList.any { it.id == imeEnter.id } &&
        ...
        val multiline =
            node.inputType and InputType.TYPE_TEXT_FLAG_MULTI_LINE != 0
