...
class LocalControlServer(
    ...
    private fun handle(socket: Socket) {
        ...
        if (!headers["host"].equals(expectedHost, ignoreCase = true)) {
            return respondJson(socket, 403, JSONObject().put("error", "invalid_host"))
        }
        if (method == "POST") {
            val expectedOrigin = "http://$expectedHost"
            if (!headers["origin"].equals(expectedOrigin, ignoreCase = true)) {
                return respondJson(socket, 403, JSONObject().put("error", "invalid_origin"))
            }
        }
    ...
    private fun beginPair(socket: Socket, body: String) {
        ...
        when (pairingManager.verify(code)) {
            PairingManager.VerifyResult.OK -> {
                ...
                pending[id] = Pending(State.PENDING, ip)
    ...
    private fun pairStatus(socket: Socket, target: String) {
        ...
        when (p.state) {
            ...
            State.APPROVED -> {
                val token = p.token ?: ""
                pending.remove(id)
                respondJson(socket, 200, JSONObject().put("status", "approved").put("token", token))
            }
        }
    }
