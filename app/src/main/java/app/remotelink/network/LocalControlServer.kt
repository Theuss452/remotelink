...
class LocalControlServer(
    ...
    fun stop() {
        ...
        pending.clear(); sessions.clear(); pairingManager.invalidate()
        ScreenCaptureService.instance?.stopAll(); workers.shutdownNow()
    }
    ...
    private fun acceptLoop(server: ServerSocket) {
        while (running.get()) {
            ...
            if (!LanPolicy.isSameSubnet(binding.address, client.inetAddress, binding.prefixLength)) {
                try { client.close() } catch (_: Exception) {}; continue
            }
            if (!slots.tryAcquire()) {
            ...
            workers.execute {
                ...
                finally { slots.release(); try { client.close() } catch (_: Exception) {} }
            }
        }
    }
    ...
    private fun handle(socket: Socket) {
        ...
        val method = parts[0].uppercase(); val target = parts[1]
        val headers = mutableMapOf<String, String>(); var totalHeaders = 0
    ...
    private inline fun withSession(socket: Socket, headers: Map<String, String>, block: (Session) -> Unit) {
        ...
        if (System.currentTimeMillis() > session.expiresAt) {
            sessions.remove(hash); ScreenCaptureService.instance?.endSession(hash)
    ...
    private fun addCandidate(socket: Socket, session: Session, body: String) {
        ...
        if (rawCandidate.isBlank() || rawCandidate.length > 8192) { session.rejectedRemoteCandidates.incrementAndGet(); return respondJson(socket, 400, JSONObject().put("error", "invalid_candidate")) }
        val normalizedCandidate = LanPolicy.normalizeRemoteIceCandidate(rawCandidate, session.remoteIp, binding)
        if (normalizedCandidate == null) { session.rejectedRemoteCandidates.incrementAndGet(); return respondJson(socket, 400, JSONObject().put("error", "invalid_candidate")) }
        val mid = if (obj.isNull("sdpMid")) null else obj.optString("sdpMid").takeIf { it.isNotEmpty() }
        val line = obj.optInt("sdpMLineIndex", -1)
        if (line < 0) { session.rejectedRemoteCandidates.incrementAndGet(); return respondJson(socket, 400, JSONObject().put("error", "invalid_candidate")) }
    ...
    private fun localCandidates(socket: Socket, session: Session) {
        val service = ScreenCaptureService.instance ?: return respondJson(socket, 409, JSONObject().put("error", "capture_not_ready"))
        val array = JSONArray()
        service.drainLocalCandidates(session.hash).filter { LanPolicy.isAllowedLocalIceCandidate(it.candidate, binding) }.forEach { c ->
    ...
    private fun webRtcState(socket: Socket, session: Session) {
        ...
        state.put("state", service?.connectionState(session.hash) ?: "closed")
            .put("captureReady", service?.isReady() == true).put("captureStarted", service?.isCaptureStarted() == true)
            .put("accessibilityReady", RemoteAccessibilityService.instance != null)
            .put("remoteCandidatesAccepted", session.acceptedRemoteCandidates.get()).put("remoteCandidatesRejected", session.rejectedRemoteCandidates.get())
        respondJson(socket, 200, state)
    }

    private fun stopSession(socket: Socket, session: Session) {
        ScreenCaptureService.instance?.endSession(session.hash); sessions.remove(session.hash)
        respondJson(socket, 200, JSONObject().put("ok", true))
    }

    private fun revokeAllSessions() {
        val hashes = sessions.keys.toList(); sessions.clear(); hashes.forEach { ScreenCaptureService.instance?.endSession(it) }
    }

    private fun cleanup() {
        val now = System.currentTimeMillis(); pending.entries.removeIf { now - it.value.created > PENDING_TTL_MS }
        val expired = sessions.values.filter { now > it.expiresAt }
        expired.forEach { sessions.remove(it.hash); ScreenCaptureService.instance?.endSession(it.hash) }
    }

    private fun asset(socket: Socket, path: String, type: String) {
        val bytes = try { context.assets.open(path).use { it.readBytes() } } catch (_: Exception) { return respond(socket, 404, "text/plain", "Not found") }
        respondBytes(socket, 200, type, bytes)
    }

    private fun parseForm(raw: String): Map<String, String> = raw.split('&').mapNotNull { part ->
        if (part.isBlank()) return@mapNotNull null
        val i = part.indexOf('='); val k = if (i >= 0) part.substring(0, i) else part; val v = if (i >= 0) part.substring(i + 1) else ""
        URLDecoder.decode(k, "UTF-8") to URLDecoder.decode(v, "UTF-8")
    }.toMap()

    private fun hashToken(token: String): String = MessageDigest.getInstance("SHA-256").digest(token.toByteArray(StandardCharsets.UTF_8)).joinToString("") { "%02x".format(it) }

    private fun readFixed(input: BufferedInputStream, length: Int): ByteArray {
        val data = ByteArray(length); var offset = 0
        while (offset < length) { val n = input.read(data, offset, length - offset); if (n < 0) throw IllegalArgumentException("unexpected end of body"); offset += n }
        return data
    }

    private fun readLineLimited(input: BufferedInputStream, max: Int): String? {
        val out = StringBuilder()
        while (out.length < max) { val b = input.read(); if (b == -1) return if (out.isEmpty()) null else out.toString(); if (b == '\n'.code) break; if (b != '\r'.code) out.append(b.toChar()) }
        if (out.length >= max) throw IllegalArgumentException("line too long")
        return out.toString()
    }

    private fun respondJson(socket: Socket, code: Int, obj: JSONObject) = respond(socket, code, "application/json; charset=utf-8", obj.toString())
    private fun respond(socket: Socket, code: Int, type: String, text: String) = respondBytes(socket, code, type, text.toByteArray(StandardCharsets.UTF_8))

    private fun respondBytes(socket: Socket, code: Int, type: String, body: ByteArray) {
        val reason = when (code) { 200 -> "OK"; 202 -> "Accepted"; 400 -> "Bad Request"; 401 -> "Unauthorized"; 403 -> "Forbidden"; 404 -> "Not Found"; 409 -> "Conflict"; 410 -> "Gone"; 413 -> "Payload Too Large"; 429 -> "Too Many Requests"; 431 -> "Request Header Fields Too Large"; 500 -> "Internal Server Error"; 503 -> "Service Unavailable"; else -> "Error" }
