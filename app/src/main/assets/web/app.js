...
el.form.addEventListener('submit',async ev=>{
  ev.preventDefault();const code=el.code.value.replace(/\D/g,'');if(code.length!==6)return setMessage('Digite os 6 dígitos.','error');
  ...
  try{
    ...
    setMessage('Aprovado. Iniciando WebRTC…','success');await startWebRtc();
  }catch(e){setMessage(e.message||'Falha de conexão.','error');el.pairButton.disabled=false;}
});
...
async function startWebRtc(){
  ...
  closePeerOnly();setConnectOverlay(true);el.title.textContent='Conectando ao Android';
  ...
  pc.ontrack=ev=>{
    const stream=ev.streams?.[0]||new MediaStream([ev.track]);el.video.srcObject=stream;el.placeholder.classList.add('hidden');el.stage.classList.add('streaming');el.title.textContent='Android conectado';
  };
  ...
  if(!r.ok){if(data.error==='capture_not_ready')throw new Error('Ative “Transmissão de tela” no aplicativo Android e tente novamente.');throw new Error(data.detail||'Falha ao criar a sessão WebRTC.');}
...
function updateConnectionState(state){
  ...
  if(state==='connected'){setConnectOverlay(false);el.disconnectTop.classList.remove('hidden');el.fullscreen.disabled=false;el.fit.disabled=false;el.stage.classList.add('connected');el.pairButton.textContent='Conectado';el.pairButton.disabled=true;}
  else if(state==='failed'||state==='closed'){setConnectOverlay(false);setControlEnabled(false);el.stage.classList.remove('connected');}
}
...
el.stage.addEventListener('pointerup',ev=>{if(!pointerStart||pointerStart.id!==ev.pointerId)return;const end=normalizedPoint(ev.clientX,ev.clientY),start=pointerStart;pointerStart=null;if(!end)return;const distPx=Math.hypot(ev.clientX-start.px,ev.clientY-start.py),duration=Math.max(80,Math.min(1500,Math.round(performance.now()-start.at)));if(distPx<10&&duration<550)sendControl({type:'tap',x:end.x,y:end.y});else sendControl({type:'swipe',x1:start.x,y1:start.y,x2:end.x,y2:end.y,duration});ev.preventDefault();});
el.stage.addEventListener('pointercancel',()=>{pointerStart=null;});el.stage.addEventListener('contextmenu',ev=>ev.preventDefault());
el.stage.addEventListener('wheel',ev=>{if(!controlReady||!el.stage.classList.contains('streaming'))return;const d=Math.sign(ev.deltaY);if(!d)return;if(d>0)sendControl({type:'swipe',x1:.5,y1:.68,x2:.5,y2:.32,duration:260});else sendControl({type:'swipe',x1:.5,y1:.32,x2:.5,y2:.68,duration:260});ev.preventDefault();},{passive:false});
